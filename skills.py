...
_register(Skill(
    ...
    name="跨线索织网",
    label="cross-thread",
    description="在同一容器的多条日记线索之间发现重复模式、演变轨迹和矛盾",
    priority=3,
    triggers=["has_history"],
    prompt="""### 技能：跨线索织网
你有权访问同一容器中的其他日记线索摘要。利用这些历史上下文：
- 发现跨条目的重复模式（同样的词、同样的结构、同样的回避）
- 追踪演变轨迹（某个话题在过去几次日记中如何变化）
- 指出矛盾（这次写的和上次写的之间的张力）
- 识别周期性模式（某些主题是否按周期出现）

引用历史内容时必须精确——只能引用实际出现在上下文中的摘要。绝不捏造历史条目。"""
))
