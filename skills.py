...
# ──────────────────────────────────────────────
# Extraction Prompt (non-streaming, post-observation call)
# ──────────────────────────────────────────────

EXTRACTION_PROMPT = """你是语言特征提取器。给定日记文本，从文本中提取语言特征，并基于现有模式档案进行累积更新。

只输出有效JSON，格式如下，不输出任何其他内容：

{
  "derived_state": {
    "affect_valence": <float, -1.0到1.0，负=消极，正=积极>,
    "affect_intensity": <float, 0.0到1.0，情绪强度>,
    "word_count": <int, 文本字数>,
    "high_frequency_words": ["仅在文本中字面出现3次及以上的词"],
    "hedge_count": <int, 软化词数量>,
    "negation_count": <int, 否定词数量>,
    "pivot_count": <int, 转折词数量>,
  ...
  "patterns_update": {
    ...
    "recurring_words": ["在多条记录中重复出现的词语"],
    "recurring_phrases": ["在多条记录中重复出现的多词短语"],
